    ...
    public class AuthenticationHandler:DelegatingHandler
    {
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                //var token = request.Headers.Contains("Authorization");
                //GetValues("Authorization").FirstOrDefault();
                //if (token != null)
                if (HttpUtility.ParseQueryString(request.RequestUri.Query).Get("Authorization")!=null)
                {
                    var token = HttpUtility.ParseQueryString(request.RequestUri.Query).Get("Authorization");
                    //string path = request.RequestUri.Query.split('?')[0];
